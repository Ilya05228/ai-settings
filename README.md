...
- **код-ревью**: уже есть агенты в плагинах (`superpowers` / `get-shit-done`); при необходимости добавить **один** отдельный skill под твой чеклист или подтянуть внешний ([Jeffallan/claude-skills](https://github.com/Jeffallan/claude-skills) — _Code Reviewer_ в секции Quality), без дублирования двух процессов ревью в одном флоу
- **ресерч в вебе**: не смешивать в один «скилл» то, что лучше делает **MCP** (поиск, браузер, fetch) — держать в плагинах `plugins/mcp-*/` и правилах агента; опционально skill только с _политикой_ (когда браузер vs grep vs API)
